# Crowdmotionpathlayer (SOP)

## What This Node Is For

`crowdmotionpathlayer` Layers additional clips/actions on crowd motion paths.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/crowdmotionpathlayer.txt`)
- Example set reviewed: yes (`examples/nodes/sop/crowdmotionpathlayer/MotionPathLayerClip.txt`)
...
- Observed: official example `MotionPathLayerClip` instantiated and cooked; representative display output was `20 pts / 20 prims` at `evaluate_animation`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_crowdmotionpathlayer` (instantiated one-by-one, then deleted before next node).
