...
`cache` Caches upstream SOP results in-memory to avoid recooking expensive branches.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/cache.txt`)
- Example set reviewed: yes (`examples/nodes/sop/cache/SlowParticles.txt`)
...
- Observed: official example `SlowParticles` instantiated and cooked; representative display output was `1 pts / 1 prims` at `cache_slow_down_particles`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq2_*_cache` (instantiated one-by-one, then deleted before next node).
