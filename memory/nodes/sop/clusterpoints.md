...
`clusterpoints` Clusters points into controllable source groups, often for sim/sourcing workflows.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/clusterpoints.txt`)
- Example set reviewed: yes (`examples/nodes/sop/clusterpoints/AnimatedSourcePoints.txt`)
...
- Observed: official example `AnimatedSourcePoints` instantiated and cooked; representative display output was `683 pts / 1 prims` at `OUT_Points`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_clusterpoints` (instantiated one-by-one, then deleted before next node).
