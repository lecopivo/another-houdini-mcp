...
`attribute` Legacy/general attribute operation node (including rename workflows).

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribute.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribute/AttributeRename.txt`)
...
- Observed: official example `AttributeRename` instantiated and cooked; representative display output was `4 pts / 1 prims` at `attribute_prman2`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribute` (instantiated one-by-one, then deleted before next node).
