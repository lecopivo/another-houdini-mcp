# Attribfromvolume (SOP)

## What This Node Is For

`attribfromvolume` Samples volume fields into geometry attributes.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribfromvolume.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribfromvolume/DeformVolume.txt`)
...
- Observed: official example `DeformVolume` instantiated and cooked; representative display output was `0 pts / 0 prims` at `(example network)`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribfromvolume` (instantiated one-by-one, then deleted before next node).
