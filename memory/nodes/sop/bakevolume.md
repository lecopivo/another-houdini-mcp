...
`bakevolume` Bakes volume lookups/results into geometry attributes/maps.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/bakevolume.txt`)
- Example set reviewed: yes (`examples/nodes/sop/bakevolume/FluffyTorus.txt`)
...
- Observed: official example `FluffyTorus` instantiated and cooked; representative display output was `4 pts / 4 prims` at `merge1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_bakevolume` (instantiated one-by-one, then deleted before next node).
