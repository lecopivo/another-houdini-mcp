...
`bend` Applies controllable bend deformation to geometry.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/bend.txt`)
- Example set reviewed: yes (`examples/nodes/sop/bend/FlounderBend.txt`)
...
- Observed: official example `FlounderBend` instantiated and cooked; representative display output was `460 pts / 480 prims` at `bend1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_bend` (instantiated one-by-one, then deleted before next node).
