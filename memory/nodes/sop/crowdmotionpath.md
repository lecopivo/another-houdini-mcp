...
`crowdmotionpath` Builds and edits crowd motion paths for clip evaluation.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/crowdmotionpath.txt`)
- Example set reviewed: yes (`examples/nodes/sop/crowdmotionpath/EditMotionPathsFromSim.txt`)
...
- Observed: official example `EditMotionPathsFromSim` instantiated and cooked; representative display output was `20 pts / 20 prims` at `evaluate_paths`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_crowdmotionpath` (instantiated one-by-one, then deleted before next node).
