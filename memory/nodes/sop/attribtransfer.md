...
`attribtransfer` Transfers attributes based on spatial proximity kernels.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribtransfer.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribtransfer/NormalsAttribTransfer.txt`)
...
- Observed: official example `NormalsAttribTransfer` instantiated and cooked; representative display output was `200 pts / 162 prims` at `merge1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribtransfer` (instantiated one-by-one, then deleted before next node).
