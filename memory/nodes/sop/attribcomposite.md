...
`attribcomposite` Composites attributes from multiple sources using blend/combine operations.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribcomposite.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribcomposite/BlendAttr.txt`)
...
- Observed: official example `BlendAttr` instantiated and cooked; representative display output was `300 pts / 243 prims` at `merge1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribcomposite` (instantiated one-by-one, then deleted before next node).
