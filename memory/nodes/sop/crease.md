...
`crease` Authors crease weights for subdivision-aware hard edges.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/crease.txt`)
- Example set reviewed: yes (`examples/nodes/sop/crease/CreaseBasic.txt`)
...
- Observed: official example `CreaseBasic` instantiated and cooked; representative display output was `386 pts / 384 prims` at `subdivide1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_crease` (instantiated one-by-one, then deleted before next node).
