...
`attribcopy` Copies selected attributes from one geometry stream to another.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribcopy.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribcopy/AttribCopyTessel.txt`)
...
- Observed: official example `AttribCopyTessel` instantiated and cooked; representative display output was `361 pts / 324 prims` at `attribcopy__my_attribute`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribcopy` (instantiated one-by-one, then deleted before next node).
