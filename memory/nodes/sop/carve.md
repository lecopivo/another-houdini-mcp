...
`carve` Extracts and animates sub-sections of curves/surfaces using parametric ranges.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/carve.txt`)
- Example set reviewed: yes (`examples/nodes/sop/carve/CarveExtractCurve.txt`)
...
- Observed: official example `CarveExtractCurve` instantiated and cooked; representative display output was `20 pts / 20 prims` at `copy_circles_to_curves`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq2_*_carve` (instantiated one-by-one, then deleted before next node).
