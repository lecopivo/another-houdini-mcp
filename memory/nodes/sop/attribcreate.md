...
`attribcreate` Creates custom attributes with explicit names/types/values.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/attribcreate.txt`)
- Example set reviewed: yes (`examples/nodes/sop/attribcreate/CurveTexturing.txt`)
...
- Observed: official example `CurveTexturing` instantiated and cooked; representative display output was `6 pts / 1 prims` at `material1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_attribcreate` (instantiated one-by-one, then deleted before next node).
