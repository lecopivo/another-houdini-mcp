...
`convertline` Converts polygon/edge features into line/curve primitives.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/convertline.txt`)
- Example set reviewed: yes (`examples/nodes/sop/convertline/ClothingCurves.txt`)
...
- Observed: official example `ClothingCurves` instantiated and cooked; representative display output was `667 pts / 14 prims` at `convertline1`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_convertline` (instantiated one-by-one, then deleted before next node).
