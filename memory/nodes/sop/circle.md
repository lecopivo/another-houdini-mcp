...
`circle` Creates circular/arc primitives across multiple geometry types.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/circle.txt`)
- Example set reviewed: yes (`examples/nodes/sop/circle/CircleExamples.txt`)
...
- Observed: official example `CircleExamples` instantiated and cooked; representative display output was `51 pts / 4 prims` at `xform_closed_circles`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq2_*_circle` (instantiated one-by-one, then deleted before next node).
