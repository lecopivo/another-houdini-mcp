# Agentrelationship (SOP)

## What This Node Is For

`agentrelationship` Defines parent-child relationships between crowd agents for coordinated motion.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/agentrelationship.txt`)
- Example set reviewed: yes (`examples/nodes/sop/agentrelationship/AgentRelationshipBasic.txt`)
...
- Observed: official example `AgentRelationshipBasic` instantiated and cooked; representative display output was `15 pts / 15 prims` at `sim`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq_*_agentrelationship` (instantiated one-by-one, then deleted before next node).
