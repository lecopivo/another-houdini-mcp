...
`comb` Combs and edits directional grooming vectors on surface points.

## Session Status

- Status: studied
- Docs read: yes (`nodes/sop/comb.txt`)
- Example set reviewed: yes (`examples/nodes/sop/comb/CombGrass.txt`)
...
- Observed: official example `CombGrass` instantiated and cooked; representative display output was `648 pts / 324 prims` at `copy2`.
- Mismatches: none observed in this pass.

## Minimum Repro Setup

- Example loaded: `/obj/academy_seq3_*_comb` (instantiated one-by-one, then deleted before next node).
